...
// 备注：============================================================================
// 备注：SIX锁（Shared-Intent-eXclusive）- 高并发锁机制
// 备注：============================================================================
// 备注：
// 备注：【概述】
// 备注：
// 备注：SIX锁是一种三态锁机制，在传统读写锁（共享/独占）基础上增加了"意向(Intent)"状态：
// 备注：- Shared (读锁): 多个读可以并发
// 备注：- Intent (意向锁): 与其他意向锁互斥，但与读锁不互斥
// 备注：- eXclusive (写锁): 完全独占
// 备注：
// 备注：【设计目的】
// 备注：
// 备注：SIX锁专门为B树等树形结构设计，解决传统读写锁的并发瓶颈：
// 备注：- 读锁不能直接升级为写锁（会死锁）
// 备注：- 更新操作需要在整个路径上加写锁，并发度低
// 备注：- 意向锁允许在遍历时加轻量级锁，只在实际修改节点时加写锁
// 备注：
// 备注：【典型使用场景】
// 备注：
// 备注：B树更新操作：
// 备注：1. 从根节点开始遍历，对每个节点加意向锁
// 备注：2. 遍历过程中持有意向锁，不阻塞其他读操作
// 备注：3. 到达目标节点，将意向锁升级为写锁
// 备注：4. 修改完成后释放写锁和意向锁
// 备注：
// 备注：这样多个更新操作可以在B树不同分支上并发执行，只要它们不修改同一个节点。
// 备注：
// 备注：【锁升级路径】
// 备注：
// 备注：  read ──(tryupgrade)──► intent ──(lock_write)──► write
// 备注：   ▲                      │                          │
// 备注：   │                      ▼                          │
// 备注：   └──────(downgrade)────┘                    (unlock_write)
// 备注：
// 备注：【状态兼容性矩阵】
// 备注：
// 备注：          │ 读锁   意向锁   写锁
...
// 备注：【序列号与重锁机制】
// 备注：
// 备注：SIX锁内置序列号，支持"解锁-重锁"模式：
// 备注：1. 记录解锁前的序列号
// 备注：2. 执行可能阻塞的操作（如IO、内存分配）
// 备注：3. 尝试重锁，检查序列号是否变化
// 备注：4. 如序列号未变，说明数据未被修改，继续操作
// 备注：5. 如序列号变化，需要重新检查数据
// 备注：
// 备注：这种机制避免了长时间持有锁，提高了并发性能。
// 备注：
// 备注：【重入性】
// 备注：
// 备注：SIX锁支持读锁和意向锁的重入：
...
// 备注：
// 备注：普通读写锁：
// 备注：  读锁 → 必须解锁才能获取写锁 → 重新检查数据 → 修改
// 备注：
// 备注：SIX锁：
// 备注：  意向锁（遍历）→ 升级为写锁（修改）→ 降级为意向锁（继续遍历）→ 解锁
// 备注：
// 备注：优势：遍历过程不阻塞其他读操作，只在实际修改时独占。
/**
