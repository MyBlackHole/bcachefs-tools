...
// 备注：用户空间 ID 分配器（IDA）
//
// 备注：替代内核 xarray 后端的 IDA。不需要内核中 id → pointer 的翻译功能。
//
// 备注：实现：d-ary bitmap tree，使用平坦数组 + eytzinger 布局。
// 备注：  d == BITS_PER_LONG（64），每个节点是一个机器字。
// 备注：  置位（set bit）= 对应子树有空闲 ID
// 备注：  叶节点直接表示 ID（置位 = ID 空闲）
//
// 备注：扩容策略：
// 备注：  当根节点无空闲位且 max ID 支持更大的树时，
// 备注：  重新分配深一层的树。旧树成为新根的最左子树，
// 备注：  新树的其余部分初始化为全 1（所有 ID 空闲）。
// ============================================
